...
describe("PersonnelDocumentsSummaryCard — PD 130-141", () => {
  it("belge satırlarını SUNUCUDAN basar (mockup'ın sahte satırları DEĞİL)", () => {
    render(<PersonnelDocumentsSummaryCard personnelId="per-1" />);
  ...
  it("'+ Ekle' DEVRE-DIŞI kalır — uç var, form mockup'ı yok (şef kararı)", () => {
    render(<PersonnelDocumentsSummaryCard personnelId="per-1" />);

    const add = screen.getByRole("button", { name: "+ Ekle" });
    expect(add).toBeDisabled();
    expect(add).toHaveAttribute("title", DOCUMENT_ADD_PENDING_REASON);
  });
