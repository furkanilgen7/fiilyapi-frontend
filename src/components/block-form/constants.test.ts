...
describe("BE 109 pending gerekçesi GÖRÜNÜRDÜR", () => {
  it("kullanıcıya NEDEN kullanılamadığını söyleyen bir cümledir", () => {
    expect(BLOCK_BULK_UNITS_PENDING_REASON).toMatch(/toplu ünite/i);
    expect(BLOCK_BULK_UNITS_PENDING_REASON.length).toBeGreaterThan(20);
  });
});
