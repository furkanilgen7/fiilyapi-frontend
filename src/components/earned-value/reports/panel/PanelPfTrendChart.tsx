 ...
 */
export function PanelPfTrendChart({ pfTrend, pfBands, rangeLabel }: PanelPfTrendChartProps) {
  const thresholds = bandsFromReport(pfBands)?.daily ?? DEFAULT_PF_BANDS.daily;
  const geo = pfTrendGeometry(pfTrend, thresholds, dayShort);
  ...
  const shownIndex = hover ?? geo.points.length - 1;
    ...
    <div className="ev-panel-chart-head">
      ...
      <div className="ev-panel-chart">
        ...
        >
          ...
          {geo.xTicks.map((tick) => (
            <text key={tick.x} x={tick.x} y={PF_VIEW_H - 10} textAnchor="middle" className="ev-panel-chart__tick">
              {tick.label}
            </text>
          ))}
        </svg>
